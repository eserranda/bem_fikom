<table class="table" id="pemasukan_keuangan">
    ...
    <tbody>
        <tr>
            <?php $nomor = 0;
            foreach ($pemasukan_keuangan as $row) :
                $nomor++; ?>
            <td><?= $nomor ?></td>
            <td><?= $row['sumber_pemasukan']; ?></td>
            <td><?= $row['keterangan_pemasukan']; ?></td>
            ...
            <td>
                <button type="button" class="btn btn-primary btn-icon waves-effect waves-light btn-sm" title="Edit"
                    onclick="edit('<?= $row['id_pemasukan'] ?>')"><i class="ri-edit-2-line"></i></button>
                </button>

                <button type="button" class="btn btn-danger btn-icon waves-effect waves-light btn-sm" title="Hapus"
                    onclick="hapus('<?= $row['id_pemasukan'] ?>')"><i class="ri-delete-bin-5-line"></i></button>
                </button>
            </td>
...
function edit(id_pemasukan) {
    $.ajax({
        type: "get",
        url: "<?= base_url('keuangan/edit_pemasukan') ?>",
        data: {
...
function hapus(id_pemasukan) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "post",
                url: "<?= base_url('keuangan/hapus_pemasukan') ?>",
                data: {
                    id_pemasukan: id_pemasukan
                },
                dataType: "json",
